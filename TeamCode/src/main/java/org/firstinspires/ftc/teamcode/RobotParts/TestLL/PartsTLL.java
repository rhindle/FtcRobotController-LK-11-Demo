...
public class PartsTLL extends Parts {
    public PartsTLL(LinearOpMode opMode) {
        super(opMode);
    }
...
//        pinpointRobotOffset = new Position(104.55,140,0);  //jas
        ...
        controls = new ControlsTLL(this);
        drivetrain = new DrivetrainTLL(this);

        if (useIMU) imuMgr = new ImuMgr(this);
        positionMgr = new PositionMgr(this);
        autoDrive = new AutoDriveTLL(this);
        userDrive = new UserDriveTLL(this);
        tLL_Misc = new TLL_Misc(this);
        tLL_Limelight = new TLL_Limelight(this);
//        sb_Auto = new SB_Auto(this);

        if (useODO) {
            odometry = new OdometryTLL(this);
            odometry.odoFieldStart = fieldStartPosition;
            odometry.odoRobotOffset = odoRobotOffset;
        }
        if (usePinpoint) {
            pinpoint = new PinpointTLL(this);
